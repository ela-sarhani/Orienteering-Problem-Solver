"""
Command-line Runner for Dynamic Programming.
Entry point: loads JSON instance, runs DP solver, writes JSON solution, generates viewer.
Usage: python -m exact_approaches.dynamic_programming.run --input <instance.json> --output <solution.json>
"""
...
def build_parser() -> ArgumentParser:
    parser = ArgumentParser(description="Solve an Orienteering Problem instance with Dynamic Programming.")
    parser.add_argument(
        "--input",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("outputs/dynamic_programming/example_op_solution.json"),
...
def main() -> int:
    ...
    state_history = solution.metadata.get("state_history", {})
    instance_viz = solution_dict.get("visualization", {})
